          ...
          (
            'cores_load_template',
            {
              ...
              callback : function()
              {
                ...
                var core_basepath = $( '#' + current_core.replace('.', '\\.'), app.menu_element ).attr( 'data-basepath' );
                ...
                $( '.indexing dd', index_data_element )
                  .html( core_data.indexing ? 'yes' : 'no' );
                ...
                reload_button
                  ...
                  (
                    ...
                    function( event )
                    {
                      sammy.trigger( 'cores_load_data', { refresh : true } );
                      if (app.cores_data[current_core].indexing) 
                      {
                        alert( 'Cannot reload while indexing is in progress.' )
                        return false;
                      }
                ...
                $( '#actions #reindex', cores_element )
                  ...
                  (
                    ...
                    function( event )
                    {
                      ...
                      if (app.cores_data[current_core].indexing) 
                      {
                        alert( 'Cannot reindex while indexing is in progress.' )
                        return false;
                      }

                      var ret = confirm( 'Do you really want to reindex Core "' + current_core + '"?' );
                      if( !ret )
                      {
                        return false;
                      }
                      ...
                      (
                        {
                          url : app.config.solr_path + app.config.core_admin_path + '?wt=json&action=RELOAD&reindex=true&core=' + current_core,
                          dataType : 'json',
                          context : $( this ),
                          beforeSend : function( xhr, settings )
                          {
                            $( '#actions #reload', cores_element ).hide();
                            $( '#actions #reindex', cores_element ).hide();
                            $( '#actions #full-reindex', cores_element ).hide();
                            $( '#actions #loader' ).addClass( 'loader' ).html(' ??? Preparing for reindex ??? ');
                          },
                          success : function( response, text_status, xhr )
                          {
                            delete app.cores_data;
                            ...
                            (
                              function()
                              {
                                $( '#actions #loader' ).removeClass( 'loader' ).html('');
                                $( '#actions #reload', cores_element ).show();
                                $( '#actions #reindex', cores_element ).show();
                                $( '#actions #full-reindex', cores_element ).show();
                              },
                              5000
                            );
                          },
                ...
                $( '#actions #full-reindex', cores_element )
                  .die( 'click' )
                  .live
                  (
                    'click',
                    function( event )
                    {
                      ...
                      var ret = confirm( 'Do you really want to fully reindex Core "' + current_core + '"?' );
                      if( !ret )
                      {
                        return false;
                      }
                      ...
                      (
                        {
                          url : app.config.solr_path + app.config.core_admin_path + '?wt=json&action=RELOAD&reindex=true&deleteAll=true&core=' + current_core,
                          dataType : 'json',
                          context : $( this ),
                          beforeSend : function( xhr, settings )
                          {
