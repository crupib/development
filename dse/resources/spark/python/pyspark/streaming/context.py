...
Please use the DataFrames API for both improved speed and memory utilization""")
        ...
        jssc = self._jvm.CassandraPythonUtil.streamingContextPythonFunctions(self._jssc.ssc())
        jrdd = jssc.cassandraTable(keyspace, table)
        return RDD(jrdd, self._sc, BatchedSerializer(PickleSerializer())).map(lambda d: convertRow(d))
