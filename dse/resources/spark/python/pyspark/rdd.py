...
class RDD(object):
    ...
    def saveToCassandra(self, keyspace, table, columns = None):
        import logging
        logging.warning("""Use of rdd.saveToCassandraTable from pyspark is deprecated and may be removed in future releases.
Please use the DataFrames API for both improved speed and memory utilization""")
        from pyspark.sql import Row
        from uuid import UUID

        # It is not possible to directly pass UUIDs to Java, due to some
        # Kryo problems with serialization (throws NPE). Therefore, let's convert
        # UUIDs to strings.
        def convertRow(row):
            def convertColumnValue(columnValue):
                if isinstance(columnValue, UUID):
                    return str(columnValue)
                elif isinstance(columnValue, list):
                    return [convertColumnValue(v) for v in columnValue]
                elif isinstance(columnValue, set):
                    return set((convertColumnValue(v) for v in columnValue))
                elif isinstance(columnValue, dict):
                    return dict(zip(columnValue.keys(),[convertColumnValue(v) for v in columnValue.values()]))
                else:
                    return columnValue

            return dict(zip(row.keys(),[convertColumnValue(v) for v in row.values()]))

        if columns is not None:
            columns = ListConverter().convert(columns, self.ctx._gateway._gateway_client)

        # We didn't find a way how to unpickle Row objects on Java side.
        # Therefore we're converting them to dictionaries here:
        rdd = self
        first = rdd.take(1)
        if len(first) == 1 and type(first[0]) == Row:
            rdd = rdd.map(lambda row: dict(zip(row.__fields__, row)))

        rdd = rdd.map(lambda row: convertRow(row))

        savable_rdd = self.ctx._jvm.CassandraPythonUtil.rddPythonFunctions(rdd._jrdd)
        savable_rdd.saveToCassandra(keyspace, table, columns)
