...
class SparkContext(object):
    ...
    def _initialize_context(self, jconf):
        ...
        """
        ctx = self._jvm.JavaSparkContext(jconf)
        return ctx
    ...
    def cassandraTable(self, keyspace, table):
        import logging
        logging.warning("""Use of sc.cassandraTable from pyspark is deprecated and may be removed in future releases.
Please use the DataFrames API for both improved speed and memory utilization""")
        from pyspark.sql import Row

        def convertRow(row):
            return Row(**row)

        jsc = self._jvm.CassandraPythonUtil.sparkContextPythonFunctions(self._jsc.sc())
        jrdd = jsc.cassandraTable(keyspace, table)
        return RDD(jrdd, self, BatchedSerializer(PickleSerializer())).map(lambda d: convertRow(d))
