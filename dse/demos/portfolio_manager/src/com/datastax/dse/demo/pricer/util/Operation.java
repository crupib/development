 ...
 */
package com.datastax.dse.demo.pricer.util;
...
import com.datastax.dse.demo.pricer.Session;
...
public abstract class Operation
{
     ...
     */
    public abstract void run(Cassandra.Iface client) throws IOException;
    ...
    protected List<String> generateValues()
    {
        ...
        for (int i = 0; i < session.getCardinality(); i++)
        {
            ...
            values.add(Hex.bytesToHex(value));
        }
    ...
    protected String[] generatePortfolio(int limit, Cassandra.Iface client)
    {
        String[] portfolio = new String[Pricer.randomizer.nextInt(limit)+1];
        String[] foundStocks = loadStocks(client);
        for(int i=0; i<portfolio.length; i++)
        {
            portfolio[i] = pickFoundStock(foundStocks);
        }
    ...
    protected String pickFoundStock(String[] foundTickers)    
    {        
        if ( foundTickers == null || foundTickers.length == 0 ) 
            throw new IllegalArgumentException("No stocks were found. Make sure that You have inserted some data into the Stocks column family.");
        return foundTickers[Pricer.randomizer.nextInt(foundTickers.length)];
    }
    ...
    protected String[] loadStocks(Cassandra.Iface  client) 
    {
        SlicePredicate predicate = new SlicePredicate();
        predicate.setSlice_range(new SliceRange(ByteBufferUtil.EMPTY_BYTE_BUFFER, ByteBufferUtil.EMPTY_BYTE_BUFFER, false, 10));
        ...
        try
        {
            List<KeySlice> results = client.get_range_slices(new ColumnParent("Stocks"), predicate, keyRange, ConsistencyLevel.ONE);
            for ( KeySlice result : results )
            {
                stocks.add(new String(result.getKey()));
            }
        } catch (Exception e)
        {
            e.printStackTrace();
        } 
        return stocks.toArray(new String[]{});
    }
}
