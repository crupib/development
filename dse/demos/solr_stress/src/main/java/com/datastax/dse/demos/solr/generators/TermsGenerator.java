...
public class TermsGenerator implements Closeable
{
    ...
    private final HttpSolrServer sorlClient;

    public TermsGenerator(String solrUrl)
    {
        this.sorlClient = new HttpSolrServer(solrUrl);
    }

    public Map<String, List<Term>> loadTermsForFields(Iterable<String> fields, int termPerFieldLimit)
    {
    ...
    public Map<String, List<Term>> loadTermsForFields(Iterable<String> fields)
    {
        return loadTermsForFields(fields, DEFAULT_TERMS_PER_FIELD_LIMIT);
    }

    private List<Term> loadTerms(String field, int limit)
    {
